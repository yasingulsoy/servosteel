...
export type ListeSatiri = IletiOzeti & { sinif: Sinif | null; firma: { id: number; firma: string } | null };
...
export function posta(
  kutu: string,
  klasor: Klasor,
  ek: { uid?: number; sayfa?: number; yaz?: string; kime?: string } = {}
): string {
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-baseline justify-between gap-2 border-b border-line px-4 py-3">
        <h2 className="min-w-0 truncate font-semibold">
          {klasor === "gelen" ? "Gelen" : "Gönderilmiş"}
          <span className="ml-1.5 font-normal text-muted">· {kutuKisaAdi(kutu)}</span>
        </h2>
        <span className="shrink-0 text-xs text-muted">{toplam.toLocaleString("tr-TR")} ileti</span>
      </div>
      ...
      {hata ? (
        ...
        <p className="flex flex-1 flex-col items-center justify-center gap-2 p-8 text-sm text-muted">
          <MailOpen className="size-6 opacity-40" aria-hidden /> Klasör boş.
        </p>
      ) : (
        <ul className="min-h-0 flex-1 divide-y divide-line overflow-y-auto">
          {satirlar.map((m) => {
            const secili = m.uid === seciliUid;
            const okunmadi = !m.okundu && klasor === "gelen";
            const tur = m.sinif?.tur && TUR_ETIKET[m.sinif.tur] ? m.sinif.tur : null;
            const firma = m.sinif?.firma ?? m.firma?.firma ?? null;
            return (
              <li key={m.uid}>
                <Link
                  href={posta(kutu, klasor, { uid: m.uid, sayfa })}
                  aria-current={secili ? "true" : undefined}
                ...
                >
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className={`min-w-0 flex-1 truncate text-sm ${okunmadi ? "font-bold text-ink" : "font-medium"}`}>
                      {klasor === "giden" ? <span className="font-normal text-muted">Kime: </span> : null}
                      {m.kisi}
                    </span>
      ...
      {sayfaSayisi > 1 ? (
        <nav className="flex items-center justify-between border-t border-line px-4 py-2 text-sm" aria-label="Sayfalar">
          {sayfa > 1 ? (
            <Link href={posta(kutu, klasor, { sayfa: sayfa - 1 })} className="rounded-md px-2 py-1 hover:bg-surface-alt">
              ← Yeni
            </Link>
          ) : (
            <span />
  ...
  return (
    <article className="flex h-full min-h-0 flex-col">
      <header className="border-b border-line px-5 py-4">
        <Link href={posta(kutu, klasor, { sayfa })} className="mb-2 inline-block text-sm text-muted hover:text-ink lg:hidden">
          ← Listeye dön
        </Link>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {klasor === "gelen" ? (
            <Link href={posta(kutu, klasor, { uid: ileti.uid, sayfa, yaz: "yanit" })} className={dugme}>
              <CornerUpLeft className="size-4" aria-hidden /> Yanıtla
            </Link>
          ) : null}
          <Link href={posta(kutu, klasor, { uid: ileti.uid, sayfa, yaz: "ilet" })} className={dugme}>
