...
type Arama = { sekme?: string; kutu?: string; sonuc?: string; sayfa?: string };
...
export default async function GidenSayfasi({ searchParams }: { searchParams: Promise<Arama> }) {
  ...
  const giden = await gidenListesi({ kutu, sonuc: sp.sonuc }, sayfa);
  const sayfaSayisi = Math.max(1, Math.ceil(giden.toplam / GIDEN_SAYFA_BOYU));
    ...
    <Kabuk aktif="kampanya" kullanici={ben}>
      <main>
        ...
        <form className="mt-4 flex flex-wrap items-end gap-2 text-sm" action="/admin/firmalar/giden">
          ...
          <span className="ml-auto text-muted">{giden.toplam.toLocaleString("tr-TR")} kayıt</span>
        </form>
