...
updated: "2026-09-17"
summary: "Do not select a servo feeder from a single catalogue figure; check three things: peak speed covers your line, roller pressure can be tuned to your material, and synchronization with the press, including pilot release, is fully implemented. Correct sizing needs sheet thickness and grade, strip width, pitch range, press strokes per minute and coil weight. Servosteel standard feeders handle 0.4–4.0 mm thickness and widths up to 1600 mm."
faq:
  - q: "Why is average feeding speed different from peak speed?"
    a: "Average speed is pitch × strokes per minute ÷ 1000, so a 250 mm pitch at 40 strokes/min averages 10 m/min. The feeder must deliver each pitch in the short window when the press is open, so the peak speed it needs is several times the average; select on peak speed."
  - q: "When should I choose a straightener servo feeder?"
    a: "When coil set stops the strip entering the die cleanly: straightening and feeding share one frame, the line gets shorter, there is one control panel and no synchronization problem between two machines. Thin, surface-critical sheet needs 11–13 rollers, thick material 7–9."
  - q: "Is a mini servo feeder less accurate?"
    a: "No. Pitch accuracy is identical to the standard series; the mini series saves floor space and cost on narrow strip and small presses. If you may widen your strips in the near future, the standard series is safer, because scaling up a feeder means buying a new machine."
  - q: "What does die memory give you?"
    a: "Servosteel feeders store pitch, speed, ramp and pressure values for up to 250 dies. Changeover drops from minutes to seconds, and production no longer depends on a specific operator."
---
...
Strip coming off a coil carries curvature memory (coil set). If that curvature stops the material entering the die cleanly, there are two routes: This series also comes in two bodies: [Mini Straightener Servo Feeders](/makineler/dogrultmali-servo-suruculer/mini) and [Cased Straightener Servo Feeders](/makineler/dogrultmali-servo-suruculer/kasali).
...
The mini series saves floor space and cost on narrow strip and small presses. **Pitch accuracy is identical to the standard series** — "mini" does not mean "less accurate". The model lists and size ranges of the two series are on separate pages: [Mini Servo Feeders](/makineler/servo-suruculer/mini) and [Cased Servo Feeders](/makineler/servo-suruculer/kasali).
